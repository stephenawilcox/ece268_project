...
def ascii_string_to_bitarray(s: str) -> np.ndarray:
    b = s.encode('ascii')              
    byte_vals = np.frombuffer(b, dtype=np.uint8)  
    bits = ((byte_vals[:, None] >> np.arange(7, -1, -1)) & 1).astype(np.uint8)
    vec_bits = bits.reshape(-1)
    return vec_bits

# TESTS
def testStateArrayConversions():
    b = 1600
    w = b // 25
    ...
    a = bitsToStateArray(bits, w)
    print_state(a)
    st = stateArrayToBits(a, w)
    print("The state array converted to string: \n", st)

def main():
    testStateArrayConversions()
    # hash = SHA3_hash("abc")
